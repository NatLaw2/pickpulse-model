...
function EndpointCard({
  ...
  return (
    <div className="bg-white border border-[var(--color-border)] rounded-2xl overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
      ...
      {ep.curl && (
        <div className="px-5 py-3 bg-[var(--color-bg-primary)] relative group">
          <pre className="text-xs font-mono text-[var(--color-text-secondary)] whitespace-pre-wrap">{ep.curl}</pre>
          <button
            onClick={() => onCopy(ep.curl!, idx)}
            className="absolute top-2 right-3 p-1.5 rounded-lg bg-white border border-[var(--color-border)] opacity-0 group-hover:opacity-100 transition-opacity"
          >

"""CLV snapshot diagnostics: checks closing_lines density, lock/close gap, and CLV readiness.
...
def check_snapshot_density(days: int = 7) -> bool:
    """Check how many snapshots per event exist. Returns True if any event has min!=max captured_at."""
    ...
    if multi_snap_events == 0:
        print(f"\n  FAIL: All events have only 1 snapshot (min == max captured_at).")
        print(f"    CLV will be 0.0 until snap_odds_nba_near_tip creates multiple near-tip snapshots.")
        print(f"    After the unique index fix, new snapshots should accumulate every 2 minutes.")
        return False
    else:
        pct = multi_snap_events / max(single_snap_events + multi_snap_events, 1) * 100
        print(f"\n  PASS: {pct:.0f}% of events have multiple snapshots (min != max captured_at).")
        return True


def check_lock_vs_close(days: int = 7) -> bool:
    ...
    if with_odds == 0:
        print(f"\n  WARN: No locked picks have odds captured.")
...
def check_clv_readiness() -> bool:
    """Check if CLV can be computed: needs at least one event with 2+ distinct snapshots
    where the odds differ between earliest and latest snapshot."""
    since = (datetime.now(timezone.utc) - timedelta(hours=48)).isoformat()

    rows = _sb_get("/rest/v1/closing_lines", {
        "select": "event_id,captured_at,outcome_name,price,market",
        "captured_at": f"gte.{since}",
        "sport": "eq.nba",
        "market": "eq.h2h",
        "order": "event_id,captured_at.asc",
        "limit": "2000",
    })

    if not rows:
        print("  No h2h snapshots in last 48h")
        return False

    # Group by (event_id, outcome_name)
    by_key: Dict[str, List[Dict]] = {}
    for r in rows:
        key = f"{r['event_id']}|{r['outcome_name']}"
    ...
    for key, snaps in by_key.items():
        ...
        captured_times = [s["captured_at"] for s in snaps]
        if captured_times[0] == captured_times[-1]:
            continue

        clv_candidates += 1

        # Check if price moved between earliest and latest snapshot
        first_price = snaps[0].get("price")
        last_price = snaps[-1].get("price")
        if first_price is not None and last_price is not None and first_price != last_price:
            nonzero_clv += 1

    print(f"  Outcome lines with 2+ distinct timestamps: {clv_candidates}")
    print(f"  Lines where price moved (non-zero CLV): {nonzero_clv}")

    if clv_candidates == 0:
        print(f"\n  FAIL: No outcome has multiple distinct-timestamp snapshots.")
        print(f"    CLV computation impossible until the snapshot fix takes effect.")
        print(f"    After fix, wait for games within T-90 to accumulate snapshots.")
        return False

    if nonzero_clv > 0:
        print(f"\n  PASS: {nonzero_clv} lines show price movement — CLV is computable.")
        # Show an example
        for key, snaps in by_key.items():
            if len(snaps) >= 2:
                first_price = snaps[0].get("price")
                last_price = snaps[-1].get("price")
                if first_price != last_price and first_price is not None:
                    eid, outcome = key.split("|", 1)
                    print(f"    Example: {outcome} {first_price} -> {last_price} "
                          f"({snaps[0]['captured_at'][:19]} -> {snaps[-1]['captured_at'][:19]})")
                    break
        return True
    else:
        print(f"\n  WARN: {clv_candidates} lines have multiple snapshots but no price movement yet.")
        print(f"    CLV will be non-zero once odds actually move between snapshots.")
        return True  # Structure is correct even if lines haven't moved
...
def main():
    ...
    checks = [
        ("Snapshot Density (last 7d)", lambda: check_snapshot_density(days=7)),
        ("Lock vs Close Gap (last 7d)", lambda: check_lock_vs_close(days=7)),
        ("CLV Readiness (last 48h)", check_clv_readiness),
    ]
